...
class ExpediaController {
  ...
  public getProperties = async (req: Request, res: Response) => {
    ...
    logger.info(`Retrieved ${paginatedProperties.length} Expedia properties`);
    res.json(response);
  };
  ...
  public getProperty = async (req: Request, res: Response) => {
    ...
    logger.info(`Retrieved Expedia property: ${id}`);
    res.json(response);
  };
  ...
  public createProperty = async (req: Request, res: Response) => {
    ...
    const property: MockProperty = {
      id: `expedia_${Date.now()}_${Math.random().toString(36).substring(7)}`,
    ...
    websocketService.broadcastPropertyUpdate('expedia', property);
    ...
    logger.info(`Created Expedia property: ${property.id}`);
  ...
  public updateProperty = async (req: Request, res: Response) => {
    ...
    logger.info(`Updated Expedia property: ${id}`);
    res.json(response);
  };
  ...
  public deleteProperty = async (req: Request, res: Response) => {
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastPropertyUpdate('expedia', { id, deleted: true });
    ...
    logger.info(`Deleted Expedia property: ${id}`);
    res.json(response);
  };
  ...
  public getBookings = async (req: Request, res: Response) => {
    ...
    let bookings = dataGenerator.getData().bookings.filter(b => b.channel === 'expedia');
    ...
    logger.info(`Retrieved ${paginatedBookings.length} Expedia bookings`);
    res.json(response);
  };
  ...
  public getBooking = async (req: Request, res: Response) => {
    ...
    if (!booking || booking.channel !== 'expedia') {
    ...
    logger.info(`Retrieved Expedia booking: ${id}`);
    res.json(response);
  };
  ...
  public createBooking = async (req: Request, res: Response) => {
    ...
    const booking: MockBooking = {
      id: `expedia_booking_${Date.now()}_${Math.random().toString(36).substring(7)}`,
      ...
      status: 'confirmed',
      channel: 'expedia',
      channelBookingId: `EXPEDIA${Date.now()}`,
      createdAt: new Date(),
      updatedAt: new Date()
    };
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastBookingUpdate('expedia', booking);
    ...
    logger.info(`Created Expedia booking: ${booking.id}`);
    res.status(201).json(response);
  };
  ...
  public updateBooking = async (req: Request, res: Response) => {
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastBookingUpdate('expedia', updatedBooking);
    ...
    logger.info(`Updated Expedia booking: ${id}`);
    res.json(response);
  };
  ...
  public cancelBooking = async (req: Request, res: Response) => {
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastBookingUpdate('expedia', cancelledBooking);
    ...
    logger.info(`Cancelled Expedia booking: ${id}`);
    res.json(response);
  };
  ...
  public getRatePlans = async (req: Request, res: Response) => {
    ...
    logger.info(`Retrieved ${ratePlans.length} Expedia rate plans for property: ${propertyId}`);
    res.json(response);
  };
  ...
  public createRatePlan = async (req: Request, res: Response) => {
    ...
    const ratePlan: MockRatePlan = {
      id: `expedia_rate_${Date.now()}_${Math.random().toString(36).substring(7)}`,
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastRateUpdate('expedia', ratePlan);
    ...
    logger.info(`Created Expedia rate plan: ${ratePlan.id}`);
    res.status(201).json(response);
  };
  ...
  public getCalendar = async (req: Request, res: Response) => {
    ...
    logger.info(`Retrieved ${calendars.length} Expedia calendar entries for property: ${propertyId}`);
    res.json(response);
  };
  ...
  public updateCalendar = async (req: Request, res: Response) => {
    ...
    updates.forEach((update: any) => {
      ...
      if (existingCalendar) {
        ...
        const newCalendar: MockCalendar = {
          id: `expedia_cal_${Date.now()}_${Math.random().toString(36).substring(7)}`,
          propertyId,
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastCalendarUpdate('expedia', { propertyId, updates });
    ...
    logger.info(`Updated Expedia calendar for property: ${propertyId}`);
    res.json(response);
  };
}

export const expediaController = new ExpediaController();
