...
class VrboController {
  ...
  public getProperties = async (req: Request, res: Response) => {
    ...
    logger.info(`Retrieved ${paginatedProperties.length} Vrbo properties`);
    res.json(response);
  };
  ...
  public getProperty = async (req: Request, res: Response) => {
    ...
    logger.info(`Retrieved Vrbo property: ${id}`);
    res.json(response);
  };
  ...
  public createProperty = async (req: Request, res: Response) => {
    ...
    const property: MockProperty = {
      id: `vrbo_${Date.now()}_${Math.random().toString(36).substring(7)}`,
    ...
    websocketService.broadcastPropertyUpdate('vrbo', property);
    ...
    logger.info(`Created Vrbo property: ${property.id}`);
  ...
  public updateProperty = async (req: Request, res: Response) => {
    ...
    logger.info(`Updated Vrbo property: ${id}`);
    res.json(response);
  };
  ...
  public deleteProperty = async (req: Request, res: Response) => {
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastPropertyUpdate('vrbo', { id, deleted: true });
    ...
    logger.info(`Deleted Vrbo property: ${id}`);
    res.json(response);
  };
  ...
  public getBookings = async (req: Request, res: Response) => {
    ...
    let bookings = dataGenerator.getData().bookings.filter(b => b.channel === 'vrbo');
    ...
    logger.info(`Retrieved ${paginatedBookings.length} Vrbo bookings`);
    res.json(response);
  };
  ...
  public getBooking = async (req: Request, res: Response) => {
    ...
    if (!booking || booking.channel !== 'vrbo') {
    ...
    logger.info(`Retrieved Vrbo booking: ${id}`);
    res.json(response);
  };
  ...
  public createBooking = async (req: Request, res: Response) => {
    ...
    const booking: MockBooking = {
      id: `vrbo_booking_${Date.now()}_${Math.random().toString(36).substring(7)}`,
      ...
      status: 'confirmed',
      channel: 'vrbo',
      channelBookingId: `VRBO${Date.now()}`,
      createdAt: new Date(),
      updatedAt: new Date()
    };
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastBookingUpdate('vrbo', booking);
    ...
    logger.info(`Created Vrbo booking: ${booking.id}`);
    res.status(201).json(response);
  };
  ...
  public updateBooking = async (req: Request, res: Response) => {
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastBookingUpdate('vrbo', updatedBooking);
    ...
    logger.info(`Updated Vrbo booking: ${id}`);
    res.json(response);
  };
  ...
  public cancelBooking = async (req: Request, res: Response) => {
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastBookingUpdate('vrbo', cancelledBooking);
    ...
    logger.info(`Cancelled Vrbo booking: ${id}`);
    res.json(response);
  };
  ...
  public getRatePlans = async (req: Request, res: Response) => {
    ...
    logger.info(`Retrieved ${ratePlans.length} Vrbo rate plans for property: ${propertyId}`);
    res.json(response);
  };
  ...
  public createRatePlan = async (req: Request, res: Response) => {
    ...
    const ratePlan: MockRatePlan = {
      id: `vrbo_rate_${Date.now()}_${Math.random().toString(36).substring(7)}`,
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastRateUpdate('vrbo', ratePlan);
    ...
    logger.info(`Created Vrbo rate plan: ${ratePlan.id}`);
    res.status(201).json(response);
  };
  ...
  public getCalendar = async (req: Request, res: Response) => {
    ...
    logger.info(`Retrieved ${calendars.length} Vrbo calendar entries for property: ${propertyId}`);
    res.json(response);
  };
  ...
  public updateCalendar = async (req: Request, res: Response) => {
    ...
    updates.forEach((update: any) => {
      ...
      if (existingCalendar) {
        ...
        const newCalendar: MockCalendar = {
          id: `vrbo_cal_${Date.now()}_${Math.random().toString(36).substring(7)}`,
          propertyId,
    ...
    // Broadcast update via WebSocket
    websocketService.broadcastCalendarUpdate('vrbo', { propertyId, updates });
    ...
    logger.info(`Updated Vrbo calendar for property: ${propertyId}`);
    res.json(response);
  };
}

export const vrboController = new VrboController();
